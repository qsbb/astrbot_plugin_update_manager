...
- 页面入口：AstrBot Plugin Page 的“模块运营中心”页面；实现目录为 `pages/manager/`，页面能力不可用时命令与调度仍可用。
- 模块运营中心：由“模块运营中心”页打开插件自有的独立 WebUI；资源位于 `webui/`，不会被 AstrBot 当成第二个 Plugin Page。首屏永远是登录页，不提供浏览器注册。管理员账号可由“核” Page 的“控制中心管理员”区域或独立 WebUI 的 owner 会话创建、修改、重置密码和启停；两种入口共用同一份本地账户与规则。
...
- 自动备份（走 AstrBot 官方导出器 `AstrBotExporter`）：独立开关 / 独立时间 / 独立时区，生成完整备份 zip（主库、知识库元数据与向量、插件与数据目录、配置、模板与 WebChat 等）；支持指定保存目录（留空 = AstrBot 官方默认 `data/backups`，目录必须是可写绝对路径且不能落在插件/数据目录内部）；若目录其实落在容器文件系统上（不是挂载卷），界面会明确告警「容器重建/升级后备份会丢失」。核**不会自动清理**备份文件，页面显示占用并支持手动删除（二次确认），且会标出被中断的截断文件（标记「可能不完整」，别当恢复点用）；备份期间 AstrBot 会短暂变慢（官方导出器没有异步打包），默认时间 03:30。只做备份，不含恢复。
...
独立 WebUI 默认开启并监听 `0.0.0.0:25528`，但所有管理 API 仍要求使用“核” Page 创建的管理员账户登录。Page 顶部会持续显示当前 URL、端口和运行状态，并按当前 Dashboard 主机生成可访问地址；新窗口被宿主 sandbox 阻止时，可使用“在当前面板打开”直接导航当前 iframe，浏览器后退即可返回 Plugin Page。跨网段或反向代理部署应显式填写 `webui_public_url`，并配合防火墙或 HTTPS 反向代理。端口、监听地址或开关变更后需要单插件重载“核”，不要把端口直接暴露到公网。

独立 WebUI 的左侧工作区包含模块总览、系列接管、系列推荐、每日规则、镜像加速、运行诊断、更新与回滚、备份、全局设置和安全与账户十个视图；模块名称可进入该模块的运行详情。“系列接管”视图显示统一接管模式（`native`/`managed`）、全局 revision、各可信模块的运行来源与状态原因，并可查看模块通过 `series.control@1.0` 声明的可管理字段（秘密字段只标记敏感性，不回显默认值）；仅 owner 角色可切换统一接管开关，字段写入与重置经接管 API 进行，要求 admin 及以上角色并携带 revision 做冲突保护。核只保存覆盖层，关闭接管后插件自身配置立即恢复生效。全局设置视图读取 `series.model_router@1.0` 的有效来源和可用状态，并按 `_conf_schema.json` 提供完整配置编辑；密钥只写不回显，Page 与独立 WebUI 共用同一校验、持久化与运行时应用内核。每日规则、镜像测速、系列推荐、管理员 CRUD 和统一接管面板均可在独立 WebUI 完成；高风险生命周期与回滚仍只允许 owner。统一模型路由的优先级固定为“插件显式配置 → 核 → AstrBot 原生配置”。**备份**是独立的一级视图（与「诊断与日志」「设置与安全」同级）：配置自动备份、立即备份、查看实时进度与备份列表（含占用），手动删除（二次确认，核不会自动清理），并显示上次备份成功时间。
...
在支持 Plugin Page 的 AstrBot 版本中，可从插件详情进入“模块运营中心”页面：

- **总览**：查看插件状态、AstrBot 版本、更新能力探测结果与每日规则。
- **推荐与目录**：上方范围筛选（全部 / 只看系列推荐 / 只看其他目录插件 / 只看有更新的）加关键词搜索，可信清单与本地目录合并展示，系列推荐里已出现的模块不会在目录里重复出现。系列部分覆盖知、言、序、情、境、声、临、核、枢固定可信清单；按运行时能力显示“安装/已安装”“更新”“强制更新”“启用/停用”，并提供“一键全部安装/更新”。普通更新仅在远端版本更高时可用；强制更新可用远端版本覆盖同版本或本地更高版本，并有独立二次警告；境与其他非核插件使用完全相同的安装、更新和安全门禁。批量操作仍只执行普通更新，会先检查最新状态，再串行安装未安装项、更新确有新版本的已安装项；核始终跳过自更新。完成后汇总成功/失败并刷新推荐状态、总览和目录。核有新版本时，“前往已安装插件页”优先使用宿主 bridge 跳到 `/extension#installed`；bridge 不支持时保留链接的原生 `target="_top"` 跳转，只有宿主仍不允许顶层导航时才会显示并复制更新页 URL，复制失败则弹出 URL 供手动复制。
- **设置**：四个子页签——**运行配置**（常用配置即时应用并持久化到 AstrBot `data` 目录）、**自动更新**（每日规则与总闸）、**控制中心账户**（创建/启停/改密/角色）、**镜像与网络**（加速站候选、测速、自定义站点）。
- **备份**：与「设置 / 日志」同级的顶层页签——自动备份开关/时间/时区/保存目录、立即备份、实时进度、备份列表与占用、手动删除（二次确认，核不会自动清理）。状态区显示**上次备份成功时间**（含文件名与大小）；后续备份失败只另标失败时间与原因，不会把上次成功记录顶掉。
- **其他目录插件**（推荐与目录内）：查看已安装插件、当前版本、来源与自动更新资格；「检查目录更新」按需探测全部有 GitHub 来源的插件。普通「更新」仅在检测到远端新版本时可用；「强制更新」支持以远端版本覆盖本地版本，可用于同版本重装、恢复已回退插件，或将本地更高版本覆盖为远端版本，并使用独立二次确认。进入页面或切到该 tab 都不会自动检查，避免全量探测拖慢首屏并耗尽 GitHub 匿名配额。
...
统一接管模式的设计见 [`docs/series-control-mode.md`](docs/series-control-mode.md)。该设计采用“核覆盖层 + 插件自身配置”的字段级继承模型：关闭接管时覆盖层保留但不生效，插件立即恢复自身配置；插件通过 `series.control@1.0` 声明可接管字段，核不直接读写插件私有配置。
...
每条事件都可以点击展开，查看模块、函数、行号、异常类型和完整的诊断日志正文。“核”在汇总时只会再次隐藏令牌、API key、密码、Authorization、Cookie、JWT、私钥、SSH key、Provider key 和 Bridge key 等凭据值；账号标识、邮箱、长数字、URL、路径、正文和哈希均保留。搜索同时覆盖事件码、摘要和详情。
...
| `auto_backup_enabled` | bool | false | 启用自动备份（独立于每日更新规则） |
| `auto_backup_local_time` | string | "03:30" | 自动备份时间（HH:MM，本地时间） |
| `auto_backup_timezone` | string | "Asia/Shanghai" | 自动备份使用的时区 |
| `auto_backup_dir` | string | "" | 备份保存目录；留空 = AstrBot 官方默认 `data/backups`；必须绝对路径、可创建可写，且不能落在插件/数据目录内部 |
