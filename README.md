...
独立 WebUI 默认开启并监听 `0.0.0.0:25528`，但所有管理 API 仍要求使用“核” Page 创建的管理员账户登录。Page 会按当前 Dashboard 主机生成打开地址；跨网段或反向代理部署应显式填写 `webui_public_url`，并配合防火墙或 HTTPS 反向代理。端口、监听地址或开关变更后需要单插件重载“核”，不要把端口直接暴露到公网。
