...
- 诊断会捕获本系列插件自有 logger 的 `DEBUG` 到 `CRITICAL` 事件；每个插件最多保留 1000 条，页面单次读取最多 1000 条、浏览器最多暂存 10000 条。列表每行固定先显示插件中文名，再显示时间、级别和事件，未加载插件会单独标明。
“日志”页只汇总固定可信系列，不读取枢、服务中心或其他第三方插件。各插件自己决定哪些事件值得记录：正常情况下保留初始化完成、正常卸载等关键节点，现有告警和错误自动进入缓冲；普通消息、聊天正文和高频成功路径不记录。日志只存在内存中，插件热重载或 AstrBot 重启后自动清空。

自动捕获的告警只保留模块、函数、行号和异常类型，不读取格式化后的日志正文；明确诊断事件只接受有界结构化详情。搜索同时覆盖事件码、摘要和详情。
