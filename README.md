...
- 新增“日志”页，统一读取知、言、序、情、境、声、核以及自动发现的官方系列插件所声明的 `series.diagnostics@1.0` 内存诊断；只在打开该页时增量轮询，不依赖 AstrBot 主日志。
