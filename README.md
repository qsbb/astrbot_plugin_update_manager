...
- 自动备份（走 AstrBot 官方导出器 `AstrBotExporter`）：独立开关 / 独立时间 / 独立时区，生成完整备份 zip（主库、知识库元数据与向量、插件与数据目录、配置、模板与 WebChat 等）；支持指定保存目录（留空 = AstrBot 官方默认 `data/backups`，目录必须是可写绝对路径且不能落在插件/数据目录内部）。核**不会自动清理**备份文件，页面显示占用并支持手动删除（二次确认），且会标出被中断的截断文件（标记「可能不完整」，别当恢复点用）；备份期间 AstrBot 会短暂变慢（官方导出器没有异步打包），默认时间 03:30。只做备份，不含恢复。
