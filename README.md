...
独立 WebUI 默认关闭并只监听 `127.0.0.1:25528`。需要局域网访问时，在“核”配置中显式开启 `webui_enabled`，设置 `webui_host=0.0.0.0`，并填写浏览器可访问的 `webui_public_url`；端口或监听地址变更后需要单插件重载“核”。不要把端口直接暴露到公网，生产环境应放在 HTTPS 反向代理后。
