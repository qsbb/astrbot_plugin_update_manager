...
独立 WebUI 的左侧工作区包含模块总览、系列接管、系列推荐、每日规则、镜像加速、运行诊断、更新与回滚、全局设置和安全与账户九个视图；模块名称可进入该模块的运行详情。“系列接管”视图显示统一接管模式（`native`/`managed`）、全局 revision、各可信模块的运行来源与状态原因，并可查看模块通过 `series.control@1.0` 声明的可管理字段（秘密字段只标记敏感性，不回显默认值）；仅 owner 角色可切换统一接管开关，字段写入与重置经接管 API 进行，要求 admin 及以上角色并携带 revision 做冲突保护。核只保存覆盖层，关闭接管后插件自身配置立即恢复生效。全局设置视图读取 `series.model_router@1.0` 的有效来源和可用状态，并按 `_conf_schema.json` 提供完整配置编辑；密钥只写不回显，Page 与独立 WebUI 共用同一校验、持久化与运行时应用内核。每日规则、镜像测速、系列推荐、管理员 CRUD 和统一接管面板均可在独立 WebUI 完成；高风险生命周期与回滚仍只允许 owner。统一模型路由的优先级固定为“插件显式配置 → 核 → AstrBot 原生配置”。「设置与安全」内还提供**备份**子页：独立配置自动备份、立即备份、查看实时进度与备份列表（含占用），并手动删除（二次确认，核不会自动清理）。
...
- **设置**：五个子页签——**运行配置**（常用配置即时应用并持久化到 AstrBot `data` 目录）、**自动更新**（每日规则与总闸）、**备份**（自动备份开关/时间/时区/保存目录、立即备份、实时进度、备份列表与占用、手动删除）、**控制中心账户**（创建/启停/改密/角色）、**镜像与网络**（加速站候选、测速、自定义站点）。
