...
- 当前版本：`0.5.1`；AstrBot 兼容范围：`>=4.16,<5`。
...
- **系列推荐**：查看知、言、序、情、声、核固定可信清单；按运行时能力显示“安装/已安装”“更新”“启用/停用”，并提供“一键全部安装/更新”。批量操作会先检查最新状态，再串行安装未安装项、更新确有新版本的已安装项，核始终跳过自更新；完成后汇总成功/失败并刷新推荐状态、总览和目录。核有新版本时，“前往仓库更新”优先使用宿主 bridge 导航；旧版宿主无法导航时会显示并复制仓库 URL，复制失败则弹出 URL 供手动复制。
