...
function setWebUiManualUrl(url) {
  const value = String(url || "").trim();
  const input = document.getElementById("webui-manual-url");
  const link = document.getElementById("webui-manual-open");
...
function revealWebUiUrl(url) {
  const value = String(url || "").trim();
  if (!value) return;
  setWebUiManualUrl(value);
  const box = document.getElementById("webui-manual");
  ...
  if (input) {
    try {
      ...
      console.warn("Unable to focus manual WebUI address", error);
    }
  }
}
...
async function openStandaloneWebUi() {
  ...
  try {
    ...
    console.warn("Host iframe blocked opening a new window", error);
  }
